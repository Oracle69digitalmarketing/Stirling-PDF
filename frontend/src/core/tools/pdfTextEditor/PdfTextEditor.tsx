...
const PdfTextEditor = ({ onComplete, onError }: BaseToolProps) => {
  ...
  const handleEditCertificate = useCallback(async (request: EditCertificateRequest) => {
    const formData = new FormData();
    formData.append("fileInput", request.fileInput);
    formData.append("certFile", request.certFile);
    if (request.certPassword) formData.append("certPassword", request.certPassword);
    ...
    const response = await apiClient.post("/api/v1/security/edit-certificate", formData, {
      responseType: "blob",
    });
    downloadBlob(response.data, "signed_document.pdf");
  }, []);
